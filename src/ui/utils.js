...
export function saveFile({ data, name, type }) {
  ...
  saveAs(URL.createObjectURL(new Blob([data], { type: type })), name)
}

export function saveSVGZip(svgs) {
  const zip = new JSZip()
  svgs.forEach((svg) => {
    zip.file(`${svg.name}.svg`, svg.data, { binary: false })
  })
...
export function copyToClipboard(text) {
  if (navigator.clipboard) {
    ...
    // Fallback for older browsers (less likely to be needed in Figma plugins)
...
export var prettifyXml = function (sourceXml) {
